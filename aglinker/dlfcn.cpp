...
static pthread_mutex_t gDlMutex = PTHREAD_RECURSIVE_MUTEX_INITIALIZER_NP;

#define __BIONIC_DLERROR_BUFFER_SIZE 512

static const char* __bionic_set_dlerror(char* new_value) {
  static __thread char* slot;
  char** dlerror_slot = &slot;
...
static void __bionic_format_dlerror(const char* msg, const char* detail) {
  static __thread char buffer[__BIONIC_DLERROR_BUFFER_SIZE];
...
AGLINKER_EXPORT void* aglinker_dlextlib(const char* filename, void* (*ext_dlsym)(void*, const char*)) {
  ScopedPthreadMutexLocker locker(&gDlMutex);
  soinfo* result = do_dlextlib(filename, ext_dlsym);
  if (result == NULL) {
    __bionic_format_dlerror("dlopen failed", linker_get_error_buffer());
    return NULL;
  }
...
AGLINKER_EXPORT int aglinker_dladdr(const void* addr, Dl_info* info) {
  ScopedPthreadMutexLocker locker(&gDlMutex);
...
static Elf32_Sym gLibDlSymtab[] = {
  ...
  ELF32_SYM_INITIALIZER( 0, &aglinker_dlopen, 1),
  ELF32_SYM_INITIALIZER( 7, &aglinker_dlclose, 1),
  ELF32_SYM_INITIALIZER(15, &aglinker_dlsym, 1),
  ELF32_SYM_INITIALIZER(21, &aglinker_dlerror, 1),
  ELF32_SYM_INITIALIZER(29, &aglinker_dladdr, 1),
